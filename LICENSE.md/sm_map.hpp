...
net<TYPE> :: comparitor(const node_link<TYPE> &lhs, const node_link<TYPE> &rhs)
{
...
{	
	...
	// This vector pointer holds all nodes in our net
...
net<TYPE> :: _destroy_(node<TYPE> *link, std::vector<node<TYPE>*> *visited)
{
	bool vis = false;
...
template <class TYPE> void
net<TYPE> :: addNode(TYPE key, TYPE parent, int (*func)(TYPE, TYPE), int weight)
{
...
template <class TYPE> void
net<TYPE> :: _printNode_(node<TYPE> *link, TYPE key)
{
...
// Check if a node exists in the net
template <class TYPE> node<TYPE>*
net<TYPE> :: exists(TYPE key)
{
...
template <class TYPE> node<TYPE>*
net<TYPE> :: _exists_(TYPE key, node<TYPE> *link, std::vector<node<TYPE>*> visited)
{	
	// Found the node
	if (link->data == key)
	{
